...
async function findEmbedLocations(client, containerHref, fileName) {
    ...
    try {
        ...
        const activities = await client.get(`${wpPath}/activities`);
...
export function registerAttachmentTools(server, client) {
    server.registerTool('op_list_attachments', {
        ...
        const data = await client.get(`/work_packages/${workPackageId}/attachments`);
        if (raw)
            return json(data);
        return json({
            ...paginationMeta(data),
    ...
    server.registerTool('op_get_attachment', {
        title: 'Get attachment',
        description: 'Fetch attachment metadata by id. Set saveTo to download the file content to a local path ' +
    ...
    }, async ({ id, raw, saveTo }) => tryTool(async () => {
    ...
    server.registerTool('op_upload_attachment', {
        title: 'Upload attachment',
        description: 'Upload a file from the local filesystem as an attachment on a work package. ' +
    ...
    }, async ({ workPackageId, filePath, fileName, description, embedIn, embedText }) => tryTool(async () => {
        const fileBuffer = await readFile(filePath);
        const name = fileName ?? basename(filePath);
        const metadata = { fileName: name };
        if (description)
            metadata.description = { raw: description };
        const formData = new FormData();
        formData.append('metadata', JSON.stringify(metadata));
        formData.append('file', new Blob([fileBuffer]), name);
        const attachment = await client.postFormData(`/work_packages/${workPackageId}/attachments`, formData);
        const attachmentFileName = attachment.fileName ?? name;
        ...
        if (embedIn === 'comment') {
            const commentRaw = embedText
                ? `${embedText}\n\n${imageRef}`
                : imageRef;
            const commentData = await client.post(`/work_packages/${workPackageId}/activities`, { comment: { raw: commentRaw } });
            return json({
                attachment: summarizeAttachment(attachment),
                comment: {
                    id: commentData.id,
                    createdAt: commentData.createdAt,
                    comment: commentData.comment?.raw ?? null,
                },
            });
        }
        // embedIn === 'description'
        const wp = await client.get(`/work_packages/${workPackageId}`);
        const existingDesc = wp.description?.raw ?? '';
        const newDesc = existingDesc
            ? `${existingDesc}\n\n${imageRef}`
            : imageRef;
        const updated = await client.patch(`/work_packages/${workPackageId}`, {
            lockVersion: wp.lockVersion,
            description: { raw: newDesc },
        });
        return json({
            attachment: summarizeAttachment(attachment),
            description: updated.description?.raw ?? null,
        });
    }));
    server.registerTool('op_delete_attachment', {
