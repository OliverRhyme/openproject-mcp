...
export function registerProjectTools(server: McpServer, client: OpenProjectClient) {
  server.registerTool(
    'op_list_projects',
    {
      title: 'List projects',
  ...
  server.registerTool(
    ...
    async ({ idOrIdentifier, raw }) =>
      tryTool(async () => {
        const data = await client.get<HalResource>(
          `/projects/${encodeURIComponent(idOrIdentifier)}`,
        );
  ...
  server.registerTool(
    ...
    async ({ name, identifier, description, public: isPublic, parentId }) =>
      tryTool(async () => {
        ...
        const data = await client.post<HalResource>('/projects', body);
