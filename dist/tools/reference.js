...
export function registerReferenceTools(server, client) {
    server.registerTool('op_list_types', {
        title: 'List work package types',
        description: 'List work package types (Task, Bug, Feature, Milestone, etc.). Optionally scope to a project.',
        inputSchema: {
            projectIdOrIdentifier: z.string().optional(),
        },
    }, async ({ projectIdOrIdentifier }) => tryTool(async () => {
    ...
    server.registerTool('op_list_statuses', {
        title: 'List statuses',
    ...
    server.registerTool('op_list_priorities', {
        title: 'List priorities',
        description: 'List all priorities defined on the instance.',
        inputSchema: {},
    }, async () => tryTool(async () => {
        const data = await client.get('/priorities');
        return json({
            ...paginationMeta(data),
            elements: extractElements(data).map(simpleRef),
        });
    }));
    server.registerTool('op_list_versions', {
    ...
    server.registerTool('op_api_passthrough', {
        title: 'Raw API passthrough',
        description: 'Escape hatch: call any GET endpoint under /api/v3 directly. Path should start with "/" (e.g. "/queries/42"). ' +
