export function loadConfig() {
    ...
    if (!baseUrl) {
        throw new Error('OPENPROJECT_BASE_URL is required (e.g. https://community.openproject.org)');
    }
    if (!apiKey) {
        throw new Error('OPENPROJECT_API_KEY is required. Generate one in OpenProject → My account → Access tokens → API.');
    }
    try {
        new URL(baseUrl);
    }
